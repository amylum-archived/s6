...
static inline void logdir_init (logdir_t *ap, uint32 s, uint32 n, uint32 tolerance, uint64 maxdirsize, struct taia const *retrytto, char const *processor, char const *name, unsigned int index)
{
  struct stat st ;
...
static inline int skagetln_nofill (buffer_ref b, stralloc *sa, char sep)
{
